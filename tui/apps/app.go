...
func NewModel(startDir, startFile string) *Model {
	...
	if startFile != "" { m.active = AppEditor }
...
func (m *Model) handleKey(km tea.KeyMsg) {
	if m.active == AppTerminal && m.terminalHandleKey(km) { return }
	switch km.String() {
	case "ctrl+q", "ctrl+c": m.quitting = true
	case "tab", "right": m.selectNext()
	case "shift+tab", "left": m.selectPrev()
	case "1": m.setActive(AppTerminal)
	case "2": m.setActive(AppFiles)
	case "3": m.setActive(AppEditor)
	case "4": m.setActive(AppSettings)
	case "5": m.setActive(AppTasks)
	case "r": m.refresh()
	case "enter": m.activate()
	default: m.forwardKey(km)
	}
}
...
func (m *Model) terminalHandleKey(km tea.KeyMsg) bool {
	s := km.String()
	switch s {
	case "ctrl+q": m.quitting = true; return true
	case "enter": m.runTerminalCommand(); return true
	case "backspace":
		if len(m.terminalInput) > 0 { m.terminalInput = m.terminalInput[:len(m.terminalInput)-1] }
		return true
	case "up":
		if len(m.terminalHistory) > 0 {
			if m.terminalPos < len(m.terminalHistory) { m.terminalPos++ }
			m.terminalInput = historyAt(m.terminalHistory, m.terminalPos)
		}
		return true
	case "down":
		if m.terminalPos > 0 { m.terminalPos--; m.terminalInput = historyAt(m.terminalHistory, m.terminalPos) } else { m.terminalInput = "" }
		return true
	case "esc": m.terminalInput = ""; return true
	}
	if s == "q" && m.terminalInput == "" { m.quitting = true; return true }
	if len(km.Runes) == 1 && km.Type == tea.KeyRunes { m.terminalInput += string(km.Runes); return true }
	return false
}

func historyAt(history []string, pos int) string {
	if len(history) == 0 || pos <= 0 || pos > len(history) { return "" }
	return history[len(history)-pos]
}

func (m *Model) runTerminalCommand() {
	command := strings.TrimSpace(m.terminalInput)
	if command == "" { m.terminalInput = ""; return }
	...
	if err != nil { m.terminalOut = append(m.terminalOut, m.styles.Error.Render(err.Error())) }
	if len(m.terminalOut) > 500 { m.terminalOut = m.terminalOut[len(m.terminalOut)-500:] }
	m.terminalInput = ""
}

func (m *Model) terminalDir() string {
	if m.fs != nil && m.fs.Path != "" { return m.fs.Path }
	if dir, err := os.UserHomeDir(); err == nil { return dir }
	return "/"
}
...
func (m *Model) setActive(i int) { m.active = i; if i == AppTasks { m.lastTask = m.tasks.Refresh() } }
...
func (m *Model) filesKey(km tea.KeyMsg) {
	...
	case "left": m.fs.GoUp()
	case "enter":
		if target := m.fs.Open(); target != "" { m.buf = editor.NewBuffer(target); m.edView = editor.ViewWindow{}; m.active = AppEditor }
	case "h": m.fs.Home()
	case "/": m.fs.Root()
	}
}
...
func (m *Model) editorKey(km tea.KeyMsg) {
	...
	case "enter": if m.edView.CursorRow < m.buf.Len() && m.buf.SplitLine(m.edView.CursorRow, m.edView.CursorCol) { m.edView.CursorRow++; m.edView.CursorCol = 0 }
	}
}
...
func (m *Model) editorEditKey(km tea.KeyMsg) {
	...
	case "enter": if m.buf.SplitLine(m.edView.CursorRow, m.edView.CursorCol) { m.edView.CursorRow++; m.edView.CursorCol = 0 }
	case "backspace": if m.edView.CursorCol > 0 { m.buf.DeleteAt(m.edView.CursorRow, m.edView.CursorCol-1); m.edView.CursorCol-- }
	case "ctrl+s": m.saveEditor()
	default: if len(km.Runes) == 1 && km.Type == tea.KeyRunes { m.buf.InsertAt(m.edView.CursorRow, m.edView.CursorCol, string(km.Runes[0])); m.edView.CursorCol++ }
	}
}
...
// dockItemAt is retained as a compatibility alias for older tests/tools.
